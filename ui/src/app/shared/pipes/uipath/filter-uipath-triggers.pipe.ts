...
@Pipe({
  name: 'filterUipathTriggers',
})
export class FilterUipathTriggersPipe implements PipeTransform {
  constructor(private _commonService: CommonService) {}

  transform(
    allSchedules$: Observable<IUiPathTrigger[]>
  ): Observable<IUiPathTrigger[]> {
    ...
    return combineLatest([
      ...
      map(([schedules, keyword]) => {
        let filteredScheduleList: IUiPathTrigger[] = [];
        if (keyword == '') {
          filteredScheduleList = schedules;
        } else {
          schedules.forEach((schedule: IUiPathTrigger) => {
